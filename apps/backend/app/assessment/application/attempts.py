...
class AssessmentAttemptService:
    ...
    def update_submission(
        self, attempt_id: UUID, definition_id: UUID, student_id: UUID, data: str | None
    ):
        return self.attempts.update(
            self._get(attempt_id, definition_id, student_id).update_submission(data)
        )

    def submit(self, attempt_id: UUID, definition_id: UUID, student_id: UUID):
        return self.attempts.update(self._get(attempt_id, definition_id, student_id).submit())

    def get(self, attempt_id: UUID, definition_id: UUID, student_id: UUID):
        return self._get(attempt_id, definition_id, student_id)

    def _get(self, attempt_id, definition_id, student_id):
